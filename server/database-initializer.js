...
function initDB(client) {
    ...
    //user_track
    db.createCollection('user_track', function (err, res) {
        if (err) throw err;
        console.log("Create user_track Collection!");
    });
    //user_watch
    db.createCollection('user_watch', function (err, res) {
        if (err) throw err;
        console.log("Create user_watch Collection!");
    });
    //movie
    ...
    //movie_evaluation
    db.createCollection('movie_evaluation', function (err, res) {
        if (err) throw err;
        console.log("Create movie_evaluation Collection!");

    });
    //other_user
    db.createCollection('other_user', function (err, res) {
        if (err) throw err;
        console.log("Create other_user Collection!");
    });
    ...
    for (var i = 0; i < movies.length; i++) {
        ...
        if (movies[i].Writer.indexOf(",") != -1) {
            writers = movies[i].Writer.split(",")
            for (var writer in writers) {
                let w = writers[writer].replace(/\(.*?\)/g, '').replace(/(^\s*)|(\s*$)/g, "")
                var wirterUser = {
                    name: w
                };
                if (check(otherPerson, w)) {
                    continue;
                } else {
                    otherPerson.push(wirterUser);
                }
            }
        } else {
            if (check(otherPerson, movies[i].Writer.replace(/\(.*?\)/g, '').replace(/(^\s*)|(\s*$)/g, ""))) {
                console.log(movies[i].Writer.replace(/\(.*?\)/g, ''))
                continue;
            } else {
                otherPerson.push(movies[i].Writer.replace(/\(.*?\)/g, '').replace(/(^\s*)|(\s*$)/g, ""));
            }
        }
        actors = movies[i].Actors.split(",")

        for (var actor in actors) {
            let a = actors[actor].replace(/\(.*?\)/g, '').replace(/(^\s*)|(\s*$)/g, "")
            var actorUser = {
                "name": a
            };
            if (check(otherPerson, a)) {
                continue;
            } else {
                otherPerson.push(actorUser);
            }
        }

        var directorUser = {
            name: movies[i].Director.replace(/\(.*?\)/g, '').replace(/(^\s*)|(\s*$)/g, "")
        };
        if (check(otherPerson, movies[i].Director.replace(/\(.*?\)/g, '').replace(/(^\s*)|(\s*$)/g, ""))) {
               console.log(movies[i].Director.replace(/\(.*?\)/g, ''))
            continue;
        } else {
            otherPerson.push(directorUser);
        }
    }
    //Insert all personnel information
    db.collection("other_user").insertMany(otherPerson, function (err, result) {
        if (err) throw err;
        console.log("Successfuly inserted " + result.insertedCount + " otherUser.")
